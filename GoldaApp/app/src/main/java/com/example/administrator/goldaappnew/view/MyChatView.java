...
public class MyChatView extends View {
    ...
    //-------------必须给的数据相关-------------
    private String[] str = new String[]{"门头店招", "户外广告"};
    //分配比例大小，总比例大小为100,由于经过运算后最后会是99.55左右的数值，导致圆不能够重合，会留出点空白，所以这里的总比例大小我们用101
    private float[] strPercent = new float[]{100.0f, 0.0f};
    private int count = 0;//总数
    //圆的直径
    private float mRadius = 480;
    //圆的粗细
    ...
    public float[] getStrPercent() {
        return strPercent;
    }

    public void setStrPercent(float[] strPercent) {
    ...
    private void initPaint() {
        ...
        //填充圆画笔
        inCyclePaint = new Paint();
        inCyclePaint.setAntiAlias(true);
        inCyclePaint.setStyle(Paint.Style.FILL);
        inCyclePaint.setColor(inColor);
        //文字画笔
        textPaint = new Paint();
    ...
    private void drawCycle(Canvas canvas) {
        ...
        for (int i = 0; i < strPercent.length; i++) {
            ...
            canvas.drawArc(new RectF(0, 0, mRadius, mRadius), startPercent, sweepPercent, true, cyclePaint);
        }
        canvas.drawOval(new RectF(mRadius / 8, mRadius / 8, mRadius * 7 / 8, mRadius * 7 / 8), inCyclePaint);
    }
    ...
    private void drawTextAndLabel(Canvas canvas) {
        for (int i = 0; i < strPercent.length; i++) {
            ...
            canvas.drawText(str[i] + " " + strPercent[i] + "%", mRadius + 60, i * 40, textPaint);
    ...
    private void drawTextAndSqure(Canvas canvas) {
        canvas.drawText("广告总数  " + count, -160, 0, textPaint);
        labelPaint.setColor(0xFF000000);
        canvas.drawCircle(-180, -5, 10, labelPaint);
    }
